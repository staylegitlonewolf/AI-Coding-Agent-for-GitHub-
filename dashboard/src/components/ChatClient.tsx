...
export function ChatClient({ owner, repo, defaultBranch }: { owner: string; repo: string; defaultBranch: string }) {
  const [messages, setMessages] = useState([{ role: "assistant", content: `Hello! I am your AI agent for ${repo}. Tell me what you'd like to build, fix, or add, and I'll generate the code and push a Pull Request directly to ${defaultBranch}.` }]);
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages([...newMsgs, { role: "assistant", content: data.reply || "Something went wrong." }]);
    } catch (e) {
       setMessages([...newMsgs, { role: "assistant", content: "Error connecting to Agent API." }]);
    } finally {
      setLoading(false);
    }
  };
